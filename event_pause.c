...
{
    if (pos_mouse.x >= game->pause->pos_res.x && pos_mouse.x <= \
game->pause->pos_res.x + 240&& pos_mouse.y >= game->pause->pos_res.y && \
        pos_mouse.y <= game->pause->pos_res.y + 120) {
...
{
    if (pos_mouse.x >= game->pause->pos_quit.x && pos_mouse.x <= \
game->pause->pos_quit.x + 240 && pos_mouse.y >= game->pause->pos_quit.y && \
        pos_mouse.y <= game->pause->pos_quit.y + 120
) {
...
{
    if (pos_mouse.x >= game->pause->pos_con.x && pos_mouse.x <= \
game->pause->pos_con.x + 240 && pos_mouse.y >= game->pause->pos_con.y && \
        pos_mouse.y <= game->pause->pos_con.y + 120) {
...
void check_pause_event(global *game)
{
    ...
    while (sfRenderWindow_pollEvent(game->window, &game->event)) {
        ...
        if (game->pause_is_open == 0 && sfKeyboard_isKeyPressed(sfKeyEscape)\
            && game->secs != 3) {
            sfSound_play(game->sounds->click);
